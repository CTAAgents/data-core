...
### 周期转换降级链（v1.3.0 新增，v2.1.0 增强）

| 层级 | 实现 | 优先级 | 说明 |
|:-----|:-----|:-------|:-----|
| P0 | resampler + ohlcv_aggregator | 原生实现 | OHLCV 正确聚合，纯计算实现 |
| P1 | auto_detector | auto 模式 | 自动检测输入周期，可选降级到手动指定 |
| P2 | API 层自动转换降级（v2.1.0） | API 层 | API 层转换失败时返回原始数据 |

**降级保证**: 周期转换为纯计算实现，始终可用，auto 模式失败时降级到手动指定周期。

### API 层周期自动转换降级（v2.1.0 新增）

UnifiedDataProvider.get() 对 OHLCV 类型数据的自动重采样采用三级降级策略：

| 层级 | 模式 | 说明 |
|:-----|:-----|:-----|
| P0 | 自动重采样 | 请求周期 > 源周期时，自动调用 resampler 转换 |
| P1 | 源周期推断失败 | 无法推断源周期时，直接返回原始数据 |
| P2 | 转换失败降级 | resampler 转换出错时，返回原始数据 + errors 记录 |

```
OHLCV 请求 → 数据源返回 → 周期检查
  → 请求周期 > 源周期 → 自动重采样 (P0)
       → 成功: 返回转换后数据 + meta.resampled_from
       → 失败: 返回原始数据 + errors 记录 (P2)
  → 请求周期 ≤ 源周期 → 直接返回原始数据
  → 源周期无法推断 → 直接返回原始数据 (P1)
```

**降级保证**: API 层周期转换为增强功能，失败时始终返回原始数据，不影响核心数据获取能力。

### BaseTool args_schema 降级（v2.1.0 新增）

BaseTool 的 args_schema 采用可选依赖降级策略：

| 层级 | 模式 | 说明 |
|:-----|:-----|:-----|
| P0 | 完整 args_schema | pydantic 已安装时，23 个 Tool 全部配备 Pydantic BaseModel |
| P1 | 无 args_schema | pydantic 未安装时，args_schema = None，Tool 正常使用 |

```
Tool 初始化 → 检查 pydantic 依赖
  → 已安装: args_schema = Pydantic Model (P0)
  → 未安装: args_schema = None (P1)，Tool 功能不受影响
```

**降级保证**: args_schema 为可选增强，pydantic 未安装时 Tool 完全可用，仅缺失结构化参数校验。

### Prometheus 可观测性降级（v2.2.0 新增）

Prometheus 可观测性模块采用三级降级策略，prometheus_client 为可选依赖：

| 层级 | 模式 | 说明 |
|:-----|:-----|:-----|
| P0 | 原生 prometheus_client | prometheus_client 已安装时，使用原生 Counter/Gauge/Histogram 实现，与 Prometheus 生态原生集成 |
| P1 | 自定义指标实现 | prometheus_client 未安装时，使用 observability.py 内置的 Counter/Gauge/Histogram 实现，11 个指标全部可用 |
| P2 | 可观测性关闭 | `DATACORE_METRICS_ENDPOINT_ENABLED=false` 时，HTTP 端点不启动，仅保留内存指标收集 |

```
指标更新请求 → 检查 prometheus_client 依赖
  → 已安装: 使用原生 Counter/Gauge/Histogram (P0)
       → /metrics 端点: prometheus_client.generate_latest() 输出
       → 原生 Prometheus 抓取兼容
  → 未安装: 使用自定义 Counter/Gauge/Histogram (P1)
       → /metrics 端点: generate_metrics() 输出自定义 exposition format
       → 与 Prometheus 抓取格式兼容（文本格式）
  → 端点禁用: 不启动 HTTP 服务器 (P2)
       → 仅内存指标更新，无 Prometheus 抓取
```

**指标降级矩阵**:
| 指标类型 | 原生 prometheus_client (P0) | 自定义实现 (P1) | 端点关闭 (P2) |
|:---------|:----------------------------|:---------------|:--------------|
| Counter（单调递增） | ✅ 原生 | ✅ 自定义 | ✅ 仅内存 |
| Gauge（可增减） | ✅ 原生 | ✅ 自定义 | ✅ 仅内存 |
| Histogram（延迟分布） | ✅ 原生桶位 | ✅ 自定义桶位 | ✅ 仅内存 |
| 装饰器埋点 | ✅ | ✅ | ✅ |
| /metrics HTTP 端点 | ✅ | ✅ | ❌ |
| Prometheus 抓取兼容 | ✅ 原生 | ✅ 文本格式 | ❌ |

**降级保证**:
- prometheus_client 为可选依赖，未安装时 observability.py 内置实现完全等价
- HTTP 端点为可选增强，关闭时仅失去 Prometheus 抓取能力，指标数据仍在内存中维护
- 装饰器埋点（observe_api_call / observe_tool_call）始终可用，不影响 API/Tool 调用性能
- 线程安全实现始终生效（threading.Lock）
- 所有指标在 P0/P1/P2 三种模式下语义一致，无需修改业务代码

**性能影响**:
- 装饰器开销：单次指标更新 < 0.1ms（加锁开销）
- HTTP 端点开销：仅在 /metrics 被抓取时产生，不影响主业务流程
- daemon 线程：HTTP 服务器在 daemon 线程运行，主进程退出时自动结束，不阻塞退出
...
| API 层周期转换超时 | 无 | 纯计算，无网络超时（v2.1.0 新增） |
| args_schema 校验超时 | 无 | Pydantic 校验为纯计算，无网络超时（v2.1.0 新增） |
| Prometheus metrics 端点超时 | 无 | HTTP 服务器响应由标准库处理，无独立超时（v2.2.0 新增） |
| 指标更新加锁等待 | <0.1ms | threading.Lock 保护，单次更新开销极小（v2.2.0 新增） |
...
API 层周期转换降级时，在 DataPayload.metadata 中标记：
- `resampled_from`: 源周期（转换成功时）
- `resample_failed`: 转换失败标记（转换失败时）
- `resample_skipped`: 跳过转换（请求周期 ≤ 源周期）

BaseTool args_schema 降级时，在 Tool 元数据中标记：
- `args_schema_available`: pydantic 已安装，args_schema 可用
- `args_schema_disabled`: pydantic 未安装，args_schema = None

Prometheus 可观测性降级时，在 observability 元数据中标记：
- `prometheus_client_native`: prometheus_client 已安装，使用原生实现（P0）
- `custom_implementation`: prometheus_client 未安装，使用自定义 Counter/Gauge/Histogram（P1）
- `metrics_endpoint_disabled`: HTTP 端点未启用，仅内存指标（P2）

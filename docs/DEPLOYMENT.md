...
# 返回: {"status": "healthy", "version": "0.6.0", "sources": {...}}
```

### 3.2 Prometheus 指标（预留）

当前 `MetricsCollector` 通过 Python API 提供指标快照。若需对接 Prometheus：
...
# 格式化输出（可通过 exporter 脚本转发到 Prometheus）
for endpoint, metrics in snapshot.items():
    print(f'datacore_calls_total{{endpoint="{endpoint}"}} {metrics["calls"]}')
    print(f'datacore_success_rate{{endpoint="{endpoint}"}} {metrics["success_rate"]}')
```
